...
with open("1103_1203_flesh_baseline_qa_inferenced_df.pkl", "rb") as f:
    df2 = pickle.load(f)
    filtered_df2 = df2[df2['prompt'].isin(df1['query'])]
    filtered_df2['real_output_readable_score'] = filtered_df2.apply(lambda row: readable(row), axis=1)

# df1.set_index('id', inplace=True)
# df2.set_index('id', inplace=True)

# 执行相减操作
df1['baseline_real_output_readable_score'] = filtered_df2['real_output_readable_score']
df1['difference'] = (df1['real_output_readable_score'] - df1['baseline_real_output_readable_score'])
...
with open(input_file, "rb") as f:
    ...
    # 计算两列之间的皮尔逊相关系数
    spearman_corr = reconstructed_df['query_max_normalized_value'].corr(reconstructed_df['real_output_readable_score'],method='pearson'
                                                                        )

    print("Pearson correlation coefficient:", spearman_corr)

    pearson = reconstructed_df['instruction_max_normalized_value'].corr(reconstructed_df['real_output_readable_score'], method='pearson')
    print("pearson Correlation coefficient: instruction_max_normalized_value", pearson)


    spearman_corr = reconstructed_df['query_max_normalized_value'].corr(reconstructed_df['real_output_readable_score'], method='spearman')
    print("Spearman Correlation coefficient: query_max_normalized_value", spearman_corr)
    pearson = reconstructed_df['instruction_max_normalized_value'].corr(reconstructed_df['real_output_readable_score'], method='spearman')
