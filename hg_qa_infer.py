...
def generate_text_with_logit(model, tokenizer, current_input, bl=True):
    ...
    outputs = model.generate(**inputs, temperature=0.01, output_logits=True, max_new_tokens=1024,
...
def similarity_method(model, tokenizer, prompt):
    ...
    with torch.no_grad():
        output_ids = model.generate(candidate_input, max_new_tokens=1024, temperature=0.1)
        response = tokenizer.batch_decode(output_ids[:, real_length:], skip_special_tokens=True)
        output_ids = model.generate(model_input["input_ids"], max_new_tokens=1024, temperature=0.1)[0]
...
def new_logit_parallel(model, tokenizer, prompt, max_new_tokens):
    ...
    tenseor_List = []
    with torch.no_grad():
        result = model.generate(model_input["input_ids"], temperature=0.1, max_new_tokens=1024,
...
def main(method,model, tokenizer,df,start,end ):
   ...
   # method = "gradient"
    ...
    inference_df.to_pickle(f"{start}_{end}_{method}_1024_llama2_qa_new_inferenced_df.pkl")
    print("\ndone the inference")
